def chart(self):
    ...
    if self.user_data.chart_type == 1:
        chart = pygal.Bar(x_label_rotation=45, title=f"Stock data for {self.user_data.stock_symbol} stocks for {self.user_data.start_date} to {self.user_data.end_date})"
    else:
